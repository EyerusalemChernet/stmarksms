...
<div class="card mb-3"><div class="card-body py-2">
  <form method="GET" class="form-row align-items-end">
    ...
    <div class="col-md-3 col-12 mb-2 d-flex align-items-end gap-1">
      ...
      <a href="{{ route('reports.income') }}" class="btn btn-light btn-sm">Reset</a>
      <a href="{{ route('reports.income') }}?date_from={{ $date_from }}&date_to={{ $date_to }}&export=csv" class="btn btn-success btn-sm"><i class="bi bi-download mr-1"></i>CSV</a>
    </div>
  </form>
</div></div>
...
<div class="row mb-3">
  <div class="col-md-4">
    <div class="card border-0 shadow-sm"><div class="card-body py-3">
      <div style="font-size:11px;color:#94a3b8">TOTAL INCOME</div>
      <div style="font-size:24px;font-weight:700;color:#22c55e">ETB {{ number_format($total,2) }}</div>
    ...
    <table class="table table-hover mb-0" style="font-size:13px;">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $row['count'] }}</td>
          <td class="text-success font-weight-bold">ETB {{ number_format($row['total'],2) }}</td>
        </tr>
        @empty
        <tr><td colspan="3" class="text-center text-muted py-4">No income data for this period.</td></tr>
        @endforelse
      </tbody>
    </table>
    </div>
