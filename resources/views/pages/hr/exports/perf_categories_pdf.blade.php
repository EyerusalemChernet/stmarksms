...
<body>
<h2>St. Mark School — Performance Score Categories</h2>
<p>
    @if($search) Search: <strong>{{ $search }}</strong> &nbsp;|&nbsp; @endif
    Total: {{ $categories->count() }} &nbsp;|&nbsp; Generated: {{ now()->format('d M Y, H:i') }}
</p>
<table>
    <thead>
        <tr><th>#</th><th>Name</th><th class="text-center">Weight</th><th>Description</th><th class="text-center">Active</th></tr>
    </thead>
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $c->description ?? '—' }}</td>
            <td style="text-align:center;">{{ $c->is_active ? 'Yes' : 'No' }}</td>
        </tr>
        @empty
        <tr><td colspan="5" style="text-align:center;color:#999;">No categories found.</td></tr>
        @endforelse
    </tbody>
</table>
